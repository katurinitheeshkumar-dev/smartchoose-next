...
(async () => {
  try {
    ...
    const htmlPath = path.resolve(__dirname, 'remove-bg.html');
    ...
    // Overwrite the logo files
    fs.writeFileSync(path.resolve(__dirname, 'public/logo.png'), buffer);
    fs.writeFileSync(path.resolve(__dirname, 'public/logo192.png'), buffer);
    fs.writeFileSync(path.resolve(__dirname, 'public/logo512.png'), buffer);
