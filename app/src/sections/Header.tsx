...
export function Header({ searchQuery, setSearchQuery, onNavigate, onSearchSelect }: HeaderProps) {
  const { settings, products, blogPosts } = useDatabase();
  ...
  const isBlogView = location.pathname.includes('/blog') || location.pathname.length > 1 && !['/about', '/jobs', '/admin', '/privacy', '/terms', '/disclosure', '/contact', '/returns', '/sitemap'].some(p => location.pathname.startsWith(p)) && !location.pathname.startsWith('/product/');
  const isJobView = location.pathname.startsWith('/jobs');
